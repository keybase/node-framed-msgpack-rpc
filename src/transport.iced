...
exports.Transport = class Transport extends Dispatch
  ...
  transport_id : () -> [ @remote_address(), @remote_port() ].join(":")
    ...
    while go
      if @is_connected() or @_explicit_close
        ...
        if err?
          ...
          go = false
      if not go and @is_connected() and @hooks?.pre_flush_queue?
        await @hooks?.pre_flush_queue {}, defer err
        if err?
          @_warn "Failed to connect due to bad hook: #{err.toString()}"
          go = true
