...
const SOCK = '/tmp/rpc.test10.sock'
...
describe('test10 - Unix domain sockets', () => {
  ...
  async function test_A() {
    const {x, c} = await connect(SOCK, 'P.1')
    ...
    res = await c.invoke_compressed(bad, COMPRESSION_TYPE_GZIP, {})
    expect(res.error).toBeTruthy()
    expect(res.error.toString()).toMatch(/unknown method/)
